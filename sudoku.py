...
def find_next_empty(puzzle):
    ...
    return None, None # if no spces in the puzzle are empty (-1)
...
def is_valid(puzzle, guess, row, col):
    ...
    if guess in row_vals:
        return False # if we've repeatred, then our guess is not valid!
...
def solve_sudoku(puzzle):
    ...
    for guess in range(1, 10): # range(1, 10) is 1, 2, 3, ... 9
        ...
        # step 5. if not valid OR if our gues does not solve the puzzle, then we need to 
